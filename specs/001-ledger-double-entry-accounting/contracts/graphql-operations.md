...
```
Dispatched by `actions.js#fetchLedgerEntries(filters, pageInfo)`; the reducer flattens `transaction.legs` into the legacy `lines` view-model and derives `partyTag`/`funderTag` from `analyticTags` by `axis.code`. Notes:
...
- `status` comes back as an integer (1/2/3 = open/locked/closed) and is normalized to strings in the reducer.

### `analytic_value` — party/funder search (US1 pickers; also US2/US3)
```graphql
...
### `accounting_periods` — User Story 4 (real backend)
```graphql
